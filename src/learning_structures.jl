...
function run_experiment!(agent::Agent, env::ClassificationEnvironment; t_warmup=0, verbose=false, kwargs...)
    ...
    trace = @NamedTuple{trial::Int, iscorrect::Bool, action::Int}[]
    for trial ∈ 1:N_trials
        @time begin
            _, iscorrect, action = @noinline run_trial!(agent, env; kwargs...)
            print("Trial completed in: ")
        end
        push!(trace, (;trial, iscorrect, action))
        if verbose
            pct_correct = round(runningperf(trace)*100; digits=4) #round(sum(row -> row.iscorrect, trace)/length(trace) * 100; digits=4)
            Response = iscorrect ? "\e[0;32mCorrect\e[0m" : "\e[0;31mFalse\e[0m"
            println("Trial = $(trial), Category choice = $(action), Response = $Response, Perf = $(pct_correct)%")
        end
        println("-----------------------------------------------------------------------------")
    end
    trace
